...
class MY_Controller extends MX_Controller {
	...
	protected $mLanguage = 'tr';
	protected $mAvailableLanguages = array("tr", "en");
...
//		var_dump($config['languages']); die();
		...
		{
			$page = $this->mMultilingual ? lang('home') : 'Ana Sayfa';
	...
	// Render template
	protected function render2($view_file, $layout = 'default')
	{
        ...
        // TODO: her sayfada son öğe olarak breadcrumba aktif sayfayı ekliyor. çok esnek değil düzeltilmeli
//		$this->push_breadcrumb($this->mPageTitle);
        ...
        // TODO: header ve footer kısımlarının load edilmesi layuout dosyasına alındı
//		$this->load->view('_base/head', $this->mViewData);
	...
	protected function render($view_file, $template = 'default')
	{

        // Tema klasör yolunu oluşturup kontrol et, yoksa hata verip işlemi durdur
        $templateDIR = $this->mConfig['template_folder'] . DIRECTORY_SEPARATOR . $this->mConfig['template'];
        if (!is_dir(APPPATH . $templateDIR))
        {
            show_error('Tema yüklenirken bir sorun oluştu. Lütfen yöneticiyle iletişime geçiniz', 500, 'Sistem Hatası');
        }
...
//		$this->push_breadcrumb($this->mPageTitle);
        ...
        $templateDIR = str_replace('views', '', $templateDIR);

		$this->mViewData['inner_view'] = $templateDIR . DIRECTORY_SEPARATOR . $view_file;
		$this->mViewData['template'] = $templateDIR . DIRECTORY_SEPARATOR;

        // TODO: header ve footer kısımlarının load edilmesi layuout dosyasına alındı
//		$this->load->view('_base/head', $this->mViewData);
//		$this->load->view('_layouts/'.$template, $this->mViewData);



        $this->load->view($templateDIR.'/layout', $this->mViewData);
	...
	public function error_404(){
        ...
        $this->render('errors/custom/error_404', 'material'); // TODO: render, 2. değişken temayı ayardan okuyacak
    }

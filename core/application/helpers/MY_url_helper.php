...
function route_link($routeName, $arg = NULL){
    /**
     *  ci_bootstrap config dosyasında 'route_site' dizisi içinden alınanlara göre çalışır
     *
     *  routeName ile kullanılacak rota seçilir
     *  arg ile rota için değişken gönderilir
     */
    $CI =& get_instance();
    $routeList = $CI->config->item('ci_bootstrap')['route_site'];

    if (array_key_exists($routeName, $routeList))
    {
        $url = $routeList[$routeName];

        if ( is_array($arg) )
        {
            $regular_expression = "{(.*?)}";
            preg_match_all("#" . $regular_expression . "#", $url, $find );

            foreach ($arg as $i => $item)
            {
                $url = str_replace($find[0][$i], $item, $url);
            }
    ...
    // TODO: düzenlenmeli adam akıllı bişey olsun, şimdilik iş görüyor
}